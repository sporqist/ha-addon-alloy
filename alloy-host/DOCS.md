...
Everything the **Grafana Alloy** add-on does - the journal to Loki, optional Home Assistant metrics - plus **host metrics** of the Home Assistant OS machine: CPU, memory, load, disk I/O, filesystem usage, memory pressure and temperatures, from Alloy's `prometheus.exporter.unix`.
